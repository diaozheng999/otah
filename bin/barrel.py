...
def clean(directory):
    ...
    for subdir in files:
        if os.path.isdir(directory+"/"+subdir) and \
            subdir not in IGNORED_DIRECTORIES:
...
def get_parser(parent=None):
    description = "Barreller for Nasi Lemak."

    if parent:
        parser = parent.add_parser("barrel", description=description)
    else:
        parser = argparse.ArgumentParser(description=description)
        
    parser.add_argument(
        "--clean",
        help='Cleans previously barrelled index.ts files.',
        action='store_true')

    parser.add_argument(
        '-r',
        '--source-root',
        help='root of the source code.',
        default=SOURCE_ROOT
    )

    parser.set_defaults(command="barrel")
    return parser
